...
import { Specialist, AppView, Language } from '../types';
...
const SpecialistGrid: React.FC<SpecialistGridProps> = ({ 
  ...
  const [sortBy, setSortBy] = useState<'fee' | 'experience' | 'none'>('none');
          ...
          >
            <div className="relative aspect-[1/1] rounded-[2rem] overflow-hidden mb-4 bg-slate-50 dark:bg-slate-900 shadow-inner">
              ...
              <div className={`absolute top-3 right-3 w-3 h-3 rounded-full border-2 border-white shadow-sm ${
                s.availability === 'available' ? 'bg-green-500' : s.availability === 'busy' ? 'bg-orange-500' : 'bg-slate-400'
              }`} />
